...
## 目的
定义 `bead-core` 的可选降色阶段：`Quantizer` / `MedianCutQuantizer` 在配色前处理 `PixelGrid`，把网格降到受限色数且保持形状不变，供 `generate_pattern` 选择性调用。
## 需求
### 需求:Quantizer trait（grid→grid 降色缝）
`bead-core` MUST 提供 `quantizer` 模块与 object-safe 的 **`Quantizer` trait**：`fn quantize(&self, grid: &PixelGrid) -> PixelGrid`。它接收一个 `PixelGrid`，返回同形状、不同色数受限的新 `PixelGrid`；trait 必须可通过 `&dyn Quantizer` / `Box<dyn Quantizer>` 使用。`quantize` 是**全函数**：不返回 `Result`、不 panic，且产出 `pixels.len() == width*height`。**空网格**（`pixels.len()==0`）必须原样返回、不 panic。`bead-core` 必须从 crate 根**重导出** `Quantizer` / `MedianCutQuantizer`。

#### 场景:量化产出形状一致的 PixelGrid
