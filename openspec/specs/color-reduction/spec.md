...
## 目的
定义 `bead-core` 的可选减色阶段：`BeadReducer` / `GreedyReducer` 在**配色之后**处理已贴板的 `BeadPattern`（调色板感知的珠色合并——把用量最少的珠色并入感知最近的保留珠色、只在真实珠色间流动、永不发明中间色），把不同珠色数降到 ≤ `max_colors` 且保持形状不变，供 `generate_pattern` 选择性调用。
## 需求
### 需求:≤N 上限语义与自然 no-op
