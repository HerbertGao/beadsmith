...
`GenerateOptions` 必须含 `width: u32`、`height: u32`、`resize: ResizeOptions`、`render: RenderOptions`，并提供 `Default`（`width/height` 为 `0`、
`resize`/`render` 各自的 `Default`）。它**不是** `#[non_exhaustive]`（同 `RenderOptions` 取舍）。`width==0`/`height==0` 是无效渲染，但**不**由
`GenerateOptions` 自校验——由 `image_to_grid` 的既有零维守卫在 `generate_pattern` 内确定性地返回 `Err`、不 panic。

#### 场景:默认选项填充 resize/render 而维度需调用方设置
