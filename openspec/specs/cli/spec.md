...
`bead-cli generate --input <img> --palette <json> --width <w> --height <h> --output <dir> [--max-colors <N>] [--matcher <rgb|lab|oklab>]` MUST **真实实现**：读取 `--input` 的图片字节与
...
（`bead-cli`），`bead-core` 保持无 fs。`generate` 暴露 **5 个必需 flag**（`--input`/`--palette`/`--width`/`--height`/`--output`）**加 2 个可选 flag `--max-colors <N>` 和 `--matcher <rgb|lab|oklab>`**：未给 `--max-colors` → `GenerateOptions.max_colors = None`（全调色板配色，行为与未引入降色前一致）；给 `N` → `max_colors = Some(N)`，把图案输出限制到 **≤N 种**拼豆色（见 color-reduction / pipeline 规范）；`--matcher` 不给则沿用 `MatcherKind::default()==Oklab`。CLI **不**内置品牌档位枚举——`--max-colors` 接任意 `u32`（`Some(0)` 会经 `generate_pattern` 确定性返 `Err`、非零退出，由「CLI 错误带语境」需求覆盖），help **可**提示常见档位 24/36/48/72。`cell_size`/`filter` **仍**用默认（`RenderOptions::default()` 的 `cell_size==10`、
`ResizeOptions::default()` 的 Lanczos3）、**禁止**暴露为 flag（故 `render_grid` 的 `cell_size>=5` 约束恒满足、永不触发）。

#### 场景:INIT 示例命令写出四个非空文件
- **当** 以一张有效图片、有效调色板与正整数 `--width`/`--height` 运行 `bead-cli generate ... --output <dir>`（**不给** `--max-colors` 和 `--matcher`）
- **那么** 进程退出码为 0，`<dir>` 下存在 `preview.png`、`grid.png`、`pattern.json`、`summary.txt` 四个非空文件；`pattern.json` 为**合法 UTF-8 且以 `{` 起头**
  ...
  的 `String`、逐字节相同，故 JSON 有效性经**构造传递**而非 CLI 独立解析）、`summary.txt` 首行为 `Bead Pattern Summary`；**且不给 `--max-colors` 时输出与未引入降色前完全一致**；默认 matcher 为 `oklab`
...
#### 场景:--matcher 各值成功、非法值拒绝
- **当** 分别用 `--matcher rgb`、`--matcher lab`、`--matcher oklab` 运行 `generate`（参数与输入合法，`--max-colors` 按需）
- **那么** 三者均退出码 0、写出四文件
- **当** 用 `--matcher hsv` 运行时
- **那么** 退出码为 2，stderr 含可选值提示，不 panic，不写出文件
